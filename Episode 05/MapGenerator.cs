...
public class MapGenerator : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        foreach(Room room in map)
        {
            ...
            RoomObject roomObj = newRoom.GetComponent<RoomObject>();
            roomObj.roomData = room;

            if (room.type == RoomType.Start)
            {
                player.position = newRoom.transform.position + Vector3.up;
                Camera.main.transform.position = newRoom.transform.position + Vector3.up * 12f;
            }
        }
    }
